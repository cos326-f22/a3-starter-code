...
Newton's method often converges really fast! Consider using it to calculate
`sqrt(N)`:
...
If you want K bits of precision, how many iterations of Newton's method do you
have to run? Let `newton(f N,N,i)` be the result of running `i` iterations of
Newton's method on that function `(f N)` with starting point `N`. Now calculate
`E(i) = -lg(abs(newton(f N, N, i) - sqrt(N)))`, where `lg` is the binary
logarithm. That tells you how many bits of accuracy you have after `i`
iterations. How does `E` grow as a function of `i`? How many iterations do you
need to calculate to 56 bits of accuracy, which the length of the mantissa of
double-precision floating point? That's how your computer's sqrt function
actually works, by the way.

**How many points is this "optional" thing worth?**

Perhaps none at all. Some things you do because they are intellectually
interesting. If it's not interesting, or if you're pressed for time, leave it
out.
-->
